...
const Userdit = () => {
  ...
  const getUser = async () => {
    try {
      const res = await fetch(`https://reham-api-v1.herokuapp.com/api/v1/users/${id}`);
  ...
  const getUserBills = async () => {
    try {
      const res = await fetch(`https://reham-api-v1.herokuapp.com/api/v1/bills/userbills/${id}`);
      const data = await res.json();
  ...
  const getUserResults = async () => {
    try {
      const res = await fetch(`https://reham-api-v1.herokuapp.com/api/v1/results/userres/${id}`);
      const data = await res.json();
  ...
  const updateUser = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`https://reham-api-v1.herokuapp.com/api/v1/users/${user._id}`, {
        name,
        address,
        phone: number,
        age,
